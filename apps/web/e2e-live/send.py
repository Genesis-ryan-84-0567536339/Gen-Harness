...
async def main():
    ...
    for m in json.loads(sys.argv[1]):
        await bus.publish(BRIDGE_INBOUND, "message", {"channel": "zalo", "external_msg_id": uuid.uuid4().hex,
            "external_group_id": m["group"], "group_name": None, "sender_external_id": m["sender"],
            "sender_name": m["name"], "body_text": m["text"], "kind": "text", "mentions_self": m.get("mention", False)},
