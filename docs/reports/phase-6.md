...
2. **`compose.yaml` ghim digest chưa nhúng vào binary `genh`** — script `deploy/scripts/pin-compose-images.sh` sinh đúng `deploy/compose.release.yaml` đã thay `build:` bằng `image: …@sha256:…`, và `release.yml` đính file này vào GitHub Release, nhưng `apps/genh/internal/compose` hiện chỉ dò `deploy/compose.yaml` trên đĩa lúc chạy — CHƯA có cơ chế `go:embed` để đọc bản đã ghim digest từ chính binary. Cần một phiên riêng bổ sung `go:embed` trong `internal/compose`.
